...
import { resendSmsVerificationCode } from "../../../../../api/authApi.js";
import { userRefreshStart } from "../../../../../redux/users/user.actions.js";
import { verifySMSForAuth } from "../../../../../api/smsAuthenticationApi.js";

const TIMER = 59;

const EnableSmsAuthenticator = ({ closeModal }) => {
  ...
  async function resendSmsApi() {
    try {
      setCounter(TIMER);
      setSendCode(false);
      let { data } = await resendSmsVerificationCode({ id: currentUser.id });
      if (data.success) {
        setSendCode(true);
        setState(data.data.verify);
      } else {
        throw new Error("Code Sending Failed Please try again");
      }
    } catch (error) {
      setloading(false);
...
export default EnableSmsAuthenticator;
