...
export const ShippingDetailsModal = ({
  ...
  const onSaveShippingDetails = () => {
    ...
    if (
      ...
      payload?.ship_to_id &&
      payload?.shipper_id &&
      payload?.stack_type_id &&
      payload?.charge_type_id &&
      payload?.notes !== ""
    ) {
      UsePost(updateOrderDetails, payload);
        ...
        <div className="flex flex-col px-1 justify-center">
          ...
          <div className="mb-3 ">
            <MaterialDropdown
              multiple={false}
              options={ship_To}
              value={shippingDetailsData?.shipToUUID}
              label={"Ship To"}
              name="shipToUUID"
              withRenderValue
              fullWidth
              onChange={handleDependanciesChange}
              userRoleToShow={shippingDetailsData.shipToName}
              error={isEmpty?.shipToUUID ? "Ship To Name is required" : ""}
              errorMsg={isEmpty?.shipToUUID ? "Ship To Name is required" : ""}
